...
SEARCH_SUBJECT =['My CuesHub Event','CuesHub prompted me for',"[Relief Study]"]
...
def create_prompt(c):
    ...
    if intervention_type == 'Targeted':
        ...
        curr_prompt=f""" 
        You are a stress intervention specialist. The user is feeling stressed due to {stressor} in {location}. Provide a single best intervention to effectively manage their stress. The intervention should include at most five clear steps, each with a practical and relevant action. Follow this structured format:
        Format:
    ...
    return intervention_type,interventions,curr_prompt
            ...
            for mail_id in mail_ids:
                ...
                for response_part in msg_data:
                    if isinstance(response_part, tuple):
                        ...
                        if email_already_processed(email_id,False):
                            ...
                            if len(info)==0:
                                ...
                                info["date"]=""
                                info["valence"]=""
                                intervention_type,interventions,curr_prompt=create_prompt(info)
                                save_extracted_words_all(email_id, email_from,receipient_name, receipient_uuid, email_date,info, "","",current_datetime,curr_prompt)
                                ...
                                save_email_id(email_id,email_datetime,email_from,receipient_name,receipient_uuid,"No stressor found (No image)","No Date found (No image)",current_datetime)
                            ...
                            if stressor_already_processed(receipient_uuid,info["Stressor"], info["date"]):
                                ...
                                save_extracted_words_all(email_id, email_from,receipient_name, receipient_uuid, email_date, info,"","",current_datetime,"")
                                save_email_id(email_id,email_datetime,email_from,receipient_name,receipient_uuid,info["Stressor"],info["date"],current_datetime)
                                
                                
                            else:
                                intervention_type,interventions,curr_prompt=create_prompt(info)
                                save_extracted_words_intervention(email_id, email_from,receipient_name, receipient_uuid, email_date, info,intervention_type,interventions,current_datetime,curr_prompt)                  
                                save_extracted_words_all(email_id, email_from,receipient_name, receipient_uuid, email_date, info,intervention_type,interventions,current_datetime,curr_prompt)
